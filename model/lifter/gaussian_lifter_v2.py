...
class GaussianLifterV2(BaseLifter):
    def __init__(
        ...
        deterministic=True,
        random_samples=0,
        use_fps=True,  # 新增：控制是否使用 FPS / 自訂取樣
        **kwargs,
    ):
        super().__init__()
        ...
        self.use_fps = use_fps  # 新增

        # ====== anchor 初始化（除了 xyz 之外） ======
        ...
        # 注意：這裡的 anchor 沒有 xyz，xyz 是之後根據 sample 出來的點再 concat
        ...
        self.register_buffer(
            "depth_bins",
            torch.linspace(1.0, 72.0, self.num_samples, dtype=torch.float),
            persistent=False,
        )
        self.register_buffer(
            "pc_start",
            torch.tensor(pc_range[:3], dtype=torch.float),
            persistent=False,
        )
    ...
    def init_random_anchors(self):
        """額外的 random anchors（完全獨立於影像 sample 出來的 xyz）"""
    ...
    def forward(self, metas, **kwargs):
        ...
        if self.initialize_backbone is not None:
            ...
            if self.initializer_img_downsample is not None:
                initialize_input = nn.functional.interpolate(
                    initialize_input,
                    scale_factor=self.initializer_img_downsample,
                    mode="bilinear",
                    align_corners=True,
                )
        ...
        # ========== 建立影像 pixel → 3D ray 上的一組 depth bins ==========
        ...
        uv = torch.stack(
            [u[None, :].expand(h, w), v[:, None].expand(h, w)], dim=-1
        )  # h, w, 2
        uv = (
            uv[None, None]
            .expand(b, n, h, w, 2)
            * metas["image_wh"][:, :, None, None]
        )  # b, n, h, w, 2

        uvd = uv.unsqueeze(4).expand(
            b, n, h, w, self.num_samples, 2
        )  # b, n, h, w, d, 2
        uvd1 = torch.cat([uvd, torch.ones_like(uvd)], dim=-1)  # ... ,4
        uvd1[..., :3] = (
            uvd1[..., :3] * self.depth_bins.view(1, 1, 1, 1, -1, 1)
        )  # multiply depth

        anchor_pts = projection_mat[:, :, None, None, None] @ uvd1[..., None]
        anchor_pts = anchor_pts.squeeze(-1)[..., :3]  # b, n, h, w, d, 3

        # ========== 建立 pixel-wise occupancy GT for depth bin ==========
        if kwargs.get("benchmarking", False):
            ...
            # 最後一個 channel 是「全部都沒 hit」的 class
            anchor_gt = torch.cat(
                [anchor_gt, ~torch.any(anchor_gt, dim=-1, keepdim=True)], dim=-1
            )

        # ========== 根據 pixel-wise depth distribution sample anchor ==========
        pdfs = torch.softmax(logits, dim=-1)
        deterministic = getattr(self, "deterministic", True)
        index, pdf_i = self.sampler.sample(
            pdfs, deterministic, self.anchors_per_pixel
        )  # b, n, h, w, a

        disable_mask = (
            pdfs.argmax(dim=-1, keepdim=True) == self.num_samples
        ).expand(-1, -1, -1, -1, self.anchors_per_pixel)
        # 從 depth bins 取出對應的 3D points
        sampled_anchor = self.sampler.gather(
            index.clamp(max=(self.num_samples - 1)), anchor_pts
        )  # b, n, h, w, a, 3

        # ========== 對每個 batch，用 FPS / random 取成固定 num_anchor 個點 ==========
        anchor_xyz = []
        for i in range(b):
            ...
            scan = cur_sampled_anchor[~cur_oob_mask]  # (M, 3)

            if scan.shape[0] == 0:
                # 萬一極端情況（全部 OOB），就隨便給一些點避免 crash
                scan = torch.zeros(
                    (self.num_anchor, 3),
                    device=anchor_pts.device,
                    dtype=anchor_pts.dtype,
                )
            ...
            if self.random_sampling:
                ...
                if scan.shape[0] < self.num_anchor:
                    ...
                    scan = scan[
                        np.random.choice(scan.shape[0], self.num_anchor, False)
                    ]
            else:
                # ====== 使用 FPS / fps_utils 的分支 ======
                if scan.shape[0] < self.num_anchor:
                    multi = int(math.ceil(self.num_anchor * 1.0 / scan.shape[0])) - 1
                    scan_ = scan.repeat(multi, 1)
                    scan_ = scan_ + torch.randn_like(scan_) * 0.1
                ...
                if kwargs.get("benchmarking", False):
                    ...
                    if self.use_fps:
                        # 這裡假設你的 fps_utils.farthest_point_sampling
                        # 介面跟原本 pointops 版是一樣的
                        scanidx = farthest_point_sampling(scan, sublens, new_sublens)
                    else:
                        # 不跑 FPS，直接用隨機 fallback
                        scanidx = fps_random_fallback(scan, self.num_anchor)
                else:
                    if self.use_fps:
                        # 和原本 CUDA 版本一樣的呼叫方式
                        ...
                        # 簡化介面：假設 fps_simple(points, k) → (k,)
                        scanidx = fps_simple(scan, self.num_anchor)
            ...
            if os.environ.get("DEBUG", "false") == "true":
                prefix = "kitti-"
                np.save(
                    f"{prefix}pred_scan.npy", scan.detach().cpu().numpy()
                )
                np.save(
                    "gt_scan_occ.npy", anchor_occ.detach().cpu().numpy()
                )
                np.save(
                    "gt_scan_pts.npy", anchor_pts.detach().cpu().numpy()
                )
                np.save(
                    "gt_occ.npy", metas["occ_label"].detach().cpu().numpy()
                )
                np.save(
                    "gt_pts.npy", metas["occ_xyz"].detach().cpu().numpy()
                )
                breakpoint()

        # ========== normalize xyz 到 [0,1]，再做 inverse sigmoid ==========
        anchor_xyz = torch.stack(anchor_xyz)  # (b, num_anchor, 3)
        ...
        if self.xyz_act == "sigmoid":
            ...
            # 如果未來你改成其他 activation，這裡可以再加分支
            xyz = anchor_xyz

        # anchor: [b, num_anchor, 3 + (scale+rot+opa+semantics)]
        anchor = torch.cat(
            [xyz, torch.tile(self.anchor[None], (b, 1, 1))],
            dim=-1,
        )
